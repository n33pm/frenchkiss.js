 ...
 */
export function compileCode(text: string): Function {
    const plural = {}
    ...
    return new Function(
        ...
        'var p=a||{}' + (size ? ',m=f?{' + pluralCode + '}:{}' : '') + ';return ' + code,
    )
}
 ...
 */
function generateCode(parts: string | any[], plural: {[x: string]: number}): string {
    const codes = []
            ...
            for (let j = 0; j < count; ++j) {
                if (isPlural) {
                    if (cases[j][0][0] === '=') {
                        ...
                        code += 'p["' + variable + '"]==' + escapeText(cases[j][0].substr(1))
                    } else {
 ...
 */
function parseBlocks(text: string | any[]): Array<any> {
    let stackSize = 0
    let fragment = ''
 ...
 */
function parseExpression(text: string): Array<any> {
